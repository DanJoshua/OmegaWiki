...
- `references/batch-mode.md` —— `/batch-ingest` 的 manifest 交接；并行安全规则与 INIT MODE 一致，父进程的 fan-in 节奏不同
...
- `wiki/graph/context_brief.md` —— REBUILD（INIT MODE / BATCH MODE 下跳过）
- `wiki/graph/open_questions.md` —— REBUILD（INIT MODE / BATCH MODE 下跳过）
...
1. 如果 `/init` 交接了 `canonical_ingest_path`，进入 **INIT MODE** 并原样消费该路径，不要重新扫描 `raw/`。详见 `references/init-mode.md`。如果是 `/batch-ingest` 交接的 `canonical_ingest_path`，则进入 **BATCH MODE**——并行安全协议相同，父进程不同。详见 `references/batch-mode.md`。
...
4. 可选 DeepXiv enrichment。如果 `fetch_deepxiv.py` 报 `deepxiv-sdk not installed`，把这条信息写进最终报告，提示用户运行 `./setup.sh`；其它 DeepXiv 失败（网络、认证、记录缺失）静默跳过：
...
INIT MODE 或 BATCH MODE 下整步跳过 —— 由上层（`/init` 或 `/batch-ingest`）在 fan-in 时统一处理 citation 与 paper-paper edge backfill。
...
INIT MODE 或 BATCH MODE 下跳过本步骤的 topic 部分——多个 subagent 同时往同一个 topic 页面追加会触发 merge 冲突，父进程（`/init` 或 `/batch-ingest`）在 fan-in 之后把 topic 内容更新交给 `/edit`。本步骤末尾的 `wiki/index.md` 追加仍然运行（`.gitattributes` 的 `merge=union` 让它并发安全）。
...
非 INIT MODE 且非 BATCH MODE 时再执行：
...
- `raw/papers/`、`raw/notes/`、`raw/web/` 归用户所有且只读。直接本地 `/ingest` 可在 `raw/tmp/` 下新增 prepared sidecar；直接 arXiv ingest 可把源归档写到 `raw/discovered/`。INIT MODE 与 BATCH MODE 下 `raw/` 全部严格只读——任何准备工作由父进程在 fan-out 之前完成。
...
- 必须假设有其他 `/ingest` 在并行 worktree 中同时运行 —— `/batch-ingest` 以同样的方式 fan-out subagent（见 `references/batch-mode.md`）。所有对共享文件（`graph/edges.jsonl`、`graph/citations.jsonl`、`index.md`、`log.md`）的写入必须经过 `tools/research_wiki.py` 或采用 append-only 语义。详见 `references/init-mode.md`。
- INIT MODE 或 BATCH MODE 下跳过 `fetch_s2.py citations`、`fetch_s2.py references`，以及 `rebuild-*` 命令 —— 由上层（`/init` 或 `/batch-ingest`）在 fan-in 后统一运行。
...
- `/batch-ingest` —— 通过 BATCH MODE 分批并行调用 `/ingest` 子代理
